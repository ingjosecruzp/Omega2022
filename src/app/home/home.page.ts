import { Component, ElementRef, ViewChild, Renderer2, ɵCodegenComponentFactoryResolver, ChangeDetectorRef } from '@angular/core';
...
.fromTo('transform', 'translate3d(0, -6vh, 0)', 'translate3d(0, 60vh, 0)');
 ...
 //  this.scrollenable=false;
      ...
      setTimeout(() => {
        this.platform.ready().then(async ()=>{
         ...
         //const pathFondo1 = `${this.file.dataDirectory}covers/fondo/${escolaridad}/1.svg`;
		     //let pathFondo1 = this.webview.convertFileSrc(`assets/img/19.svg`);
         let pathFondo1 = this.webview.convertFileSrc(`${this.file.dataDirectory}covers/fondo/${escolaridad}/1.svg?t=${timestamp}`);
         ...
         //Actualiza las diapositivas duplicadas
         setTimeout(()=>{
             swiper.loopDestroy();
             swiper.loopCreate();
             console.log('reborn!')
  ...
  ngAfterViewInit() {
   ...
   //  this.statusBar.overlaysWebView(true);
    ...
    async ionSlideTouchEndSlide() {
      ...
      if(this.selectSeccion == index) return;
    ...
    ionSlideTransitionStart() {
      setTimeout(() => {
        this.numeroclicks=1;
        //Esto se va ocupar para las notificaciones
        //this.pillMenu.nextSegment('0');
      }, 50);
    ...
    async buscarFondos(){
      try {
         const resp:any  = await this.actPortadasService.buscarFondosTemas();

         if(resp.ifUpdate)
         {
             console.log("actualiza fondo");
      ...
      this.storage.get(pathStorage).then(async (librosLocales) => { 
          ...
          librosLocales.forEach(element => {
            if(parseInt(element.VersionThumbnails) > parseInt(element.VersionThumbnailsLocal)) {
              ...
              promises.push(fileTransfer.download(url + element.RutaThumbnails, directory + element.RutaThumbnails));

              //Descarga los circulos
              promises.push(fileTransfer.download(`${url}/iconos/${element.NombreArchivo}.svg`, `${directory}/iconos/${element.NombreArchivo}.svg`));

              //Actuliza la version locak con la version remota
              //Si se descomenta esta linea la actulizacion va entrar siempre y en combinacion con indexdb es una manera de debugear desde el celular
   ...
   async abrirCalendarioSemana(item){
        ...
        buttons: [
          {
            ...
            handler: async () => {
              ...
              await this.actPortadasService.limpiarLibros();
    ...
    private subscribeToEvents(): void {
      ...
      /*this.connectivityService.appIsOnline.subscribe((status:any) =>{
  ...
  if(this.estadoArriba===false){
    this.pildora = this.pillMenu.getPillTextSelect();
    //console.log("divArriba")
    this.slideUp.lockSwipes(true);
    this.swipeUp2 = true;
